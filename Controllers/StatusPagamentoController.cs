...
public class StatusPagamentoController : ControllerBase {
    ...
    public async Task<ActionResult<List<StatusPagamento>>> Get(
        [FromServices] DataContext context)
    { 
        var model = await context.StatusPagamento.AsNoTracking().ToListAsync();
        return Ok(model);
    }
    ...
    [Route("{id:long}")]
    [Authorize(Roles="admin")]
    public async Task<ActionResult<StatusPagamento>> Get(
        long id,
        [FromServices] DataContext context)
    { 
        var model = await context.StatusPagamento
            .AsNoTracking()
        ...
        )
    {
            ...
            var model = await context.StatusPagamento.FirstOrDefaultAsync(x => x.Id.Equals(id));
            ...
            try 
            {
                context.StatusPagamento.Remove(model);
                await context.SaveChangesAsync();
