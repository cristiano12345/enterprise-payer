...
[Route("venda/detalhe")]
public class DetalheVendaController : ControllerBase {

    [HttpGet]
    [Route("")]
    [Authorize(Roles="admin")]
    public async Task<ActionResult<List<DetalheVenda>>> Get(
        [FromServices] DataContext context)
    { 
        var model = await context.DetalheVenda.AsNoTracking().ToListAsync();
        return Ok(model);
    }
    ...
    [Route("{id:long}")]
    [Authorize(Roles="admin")]
    public async Task<ActionResult<DetalheVenda>> Get(
        long id,
        [FromServices] DataContext context)
    { 
        var model = await context.DetalheVenda
            .AsNoTracking()
        ...
        )
    {
            ...
            var model = await context.DetalheVenda.FirstOrDefaultAsync(x => x.Id.Equals(id));
            ...
            try 
            {
                context.DetalheVenda.Remove(model);
                await context.SaveChangesAsync();
